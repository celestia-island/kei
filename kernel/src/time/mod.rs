...
pub(super) fn init() {
    ...
    {
        // Wire up a real clocksource (architected timer) before the clock
        // singletons are initialized, so CLOCK_MONOTONIC actually advances
        // and nanosleep/timers fire. Without it, read_time() returns a
        // frozen default and sleeping syscalls hang forever.
