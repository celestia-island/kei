...
pub fn catch_unwind<E: Exception, R, F: FnOnce() -> R>(f: F) -> Result<R, Option<E>> {
    ...
    unsafe {
        // kei patch: on the pinned nightly-2026-05-01 the `catch_unwind`
        // intrinsic returns `i32` (0 == no unwind), not `bool`; compare
        // against 0 explicitly (same idiom as unwinding 0.2.3).
